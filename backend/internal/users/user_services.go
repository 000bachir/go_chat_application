...
const secretKey string = "need_be_changed_in_env_file"
...
func (s *service) LoginUser(ctx context.Context, request *LoginUserRequest) (*LoginUserResponse, error) {
	...
	err = utils.VerifyHashedPassword(request.Password, userLoginRequest.Password)
	if err != nil {
		return &LoginUserResponse{}, err
	}
	...
	return &LoginUserResponse{accessToken: PrivateKey, Username: userLoginRequest.Username, ID: strconv.Itoa(int(userLoginRequest.ID))}, nil
}
